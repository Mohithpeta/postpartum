import { useState } from 'react';
import { ArrowLeft, Download, Plus, ZoomIn, ZoomOut } from 'lucide-react';
import { Link } from 'react-router-dom';
import {
  LineChart,
  Line,
  XAxis,
  YAxis,
  CartesianGrid,
  Tooltip,
  Legend,
  ResponsiveContainer,
} from 'recharts';

interface GlucoseReading {
  timestamp: string;
  glucoseLevel: number;
  mealContext: 'fasting' | 'before_meal' | 'after_meal' | 'bedtime';
}

interface FormData {
  glucoseLevel: string;
  mealContext: GlucoseReading['mealContext'];
  timestamp: string;
}

export function BloodGlucose() {
  const [readings, setReadings] = useState<GlucoseReading[]>([
    { timestamp: '2024-03-10 07:00', glucoseLevel: 95, mealContext: 'fasting' },
    { timestamp: '2024-03-10 13:00', glucoseLevel: 145, mealContext: 'after_meal' },
    { timestamp: '2024-03-11 07:30', glucoseLevel: 92, mealContext: 'fasting' },
  ]);

  const [formData, setFormData] = useState<FormData>({
    glucoseLevel: '',
    mealContext: 'fasting',
    timestamp: new Date().toISOString().slice(0, 16),
  });

  const [errors, setErrors] = useState<Partial<FormData>>({});
  const [zoomLevel, setZoomLevel] = useState(1);

  const validateForm = (): boolean => {
    const newErrors: Partial<FormData> = {};
    const glucoseNum = parseInt(formData.glucoseLevel);
    const timestamp = new Date(formData.timestamp);

    if (!formData.glucoseLevel || glucoseNum < 20 || glucoseNum > 600) {
      newErrors.glucoseLevel = 'Blood glucose must be between 20 and 600 mg/dL';
    }

    if (timestamp > new Date()) {
      newErrors.timestamp = 'Future timestamps are not allowed';
    }

    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validateForm()) {
      const newReading: GlucoseReading = {
        timestamp: formData.timestamp,
        glucoseLevel: parseInt(formData.glucoseLevel),
        mealContext: formData.mealContext,
      };

      setReadings([...readings, newReading].sort((a, b) => 
        new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()
      ));

      setFormData({
        glucoseLevel: '',
        mealContext: 'fasting',
        timestamp: new Date().toISOString().slice(0, 16),
      });
    }
  };

  const exportToCSV = () => {
    const csvContent = [
      'Timestamp,Glucose Level (mg/dL),Meal Context',
      ...readings.map(reading => 
        `${reading.timestamp},${reading.glucoseLevel},${reading.mealContext.replace('_', ' ')}`
      )
    ].join('\n');

    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = 'glucose-readings.csv';
    link.click();
  };

  const getGlucoseCategory = (level: number, context: GlucoseReading['mealContext']) => {
    switch (context) {
      case 'fasting':
        if (level < 70) return { color: 'text-red-500', label: 'Low' };
        if (level <= 99) return { color: 'text-green-500', label: 'Normal' };
        if (level <= 125) return { color: 'text-yellow-500', label: 'Pre-diabetic' };
        return { color: 'text-red-500', label: 'High' };
      
      case 'before_meal':
        if (level < 70) return { color: 'text-red-500', label: 'Low' };
        if (level <= 130) return { color: 'text-green-500', label: 'Normal' };
        return { color: 'text-red-500', label: 'High' };
      
      case 'after_meal':
        if (level < 70) return { color: 'text-red-500', label: 'Low' };
        if (level <= 180) return { color: 'text-green-500', label: 'Normal' };
        return { color: 'text-red-500', label: 'High' };
      
      case 'bedtime':
        if (level < 70) return { color: 'text-red-500', label: 'Low' };
        if (level <= 140) return { color: 'text-green-500', label: 'Normal' };
        return { color: 'text-red-500', label: 'High' };
    }
  };

  const formatMealContext = (context: string) => {
    return context.split('_').map(word => 
      word.charAt(0).toUpperCase() + word.slice(1)
    ).join(' ');
  };

  return (
    <div className="p-6">
      <div className="mb-6">
        <Link 
          to="/trackers" 
          className="inline-flex items-center text-gray-600 hover:text-gray-800"
        >
          <ArrowLeft className="w-4 h-4 mr-2" />
          Back to Trackers
        </Link>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Input Form */}
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <h2 className="text-xl font-semibold mb-4">Add New Reading</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Blood Glucose Level (mg/dL)
                </label>
                <input
                  type="number"
                  value={formData.glucoseLevel}
                  onChange={(e) => setFormData({ ...formData, glucoseLevel: e.target.value })}
                  className="w-full p-2 border rounded-md focus:ring-2 focus:ring-[#A32E76] focus:border-transparent"
                  placeholder="20-600 mg/dL"
                />
                {errors.glucoseLevel && (
                  <p className="text-red-500 text-sm mt-1">{errors.glucoseLevel}</p>
                )}
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Meal Context
                </label>
                <select
                  value={formData.mealContext}
                  onChange={(e) => setFormData({ 
                    ...formData, 
                    mealContext: e.target.value as GlucoseReading['mealContext']
                  })}
                  className="w-full p-2 border rounded-md focus:ring-2 focus:ring-[#A32E76] focus:border-transparent"
                >
                  <option value="fasting">Fasting</option>
                  <option value="before_meal">Before Meal</option>
                  <option value="after_meal">After Meal</option>
                  <option value="bedtime">Bedtime</option>
                </select>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Timestamp
                </label>
                <input
                  type="datetime-local"
                  value={formData.timestamp}
                  onChange={(e) => setFormData({ ...formData, timestamp: e.target.value })}
                  className="w-full p-2 border rounded-md focus:ring-2 focus:ring-[#A32E76] focus:border-transparent"
                />
                {errors.timestamp && (
                  <p className="text-red-500 text-sm mt-1">{errors.timestamp}</p>
                )}
              </div>

              <button
                type="submit"
                className="w-full bg-[#A32E76] text-white py-2 rounded-md hover:bg-[#8E2968] transition-colors flex items-center justify-center gap-2"
              >
                <Plus className="w-4 h-4" />
                Add Reading
              </button>
            </form>
          </div>
        </div>

        {/* Chart */}
        <div className="lg:col-span-2">
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold">Blood Glucose Chart</h2>
              <div className="flex gap-2">
                <button
                  onClick={() => setZoomLevel(Math.max(1, zoomLevel - 0.5))}
                  className="p-2 rounded-md hover:bg-gray-100"
                  aria-label="Zoom out"
                >
                  <ZoomOut className="w-5 h-5" />
                </button>
                <button
                  onClick={() => setZoomLevel(Math.min(2, zoomLevel + 0.5))}
                  className="p-2 rounded-md hover:bg-gray-100"
                  aria-label="Zoom in"
                >
                  <ZoomIn className="w-5 h-5" />
                </button>
                <button
                  onClick={exportToCSV}
                  className="p-2 rounded-md hover:bg-gray-100"
                  aria-label="Export data"
                >
                  <Download className="w-5 h-5" />
                </button>
              </div>
            </div>

            <div style={{ width: '100%', height: 400 }}>
              <ResponsiveContainer>
                <LineChart
                  data={readings}
                  margin={{ top: 5, right: 30, left: 20, bottom: 5 }}
                >
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis
                    dataKey="timestamp"
                    tickFormatter={(value) => new Date(value).toLocaleDateString()}
                  />
                  <YAxis domain={[0, 400]} />
                  <Tooltip
                    labelFormatter={(value) => new Date(value).toLocaleString()}
                    formatter={(value: number) => [
                      `${value} mg/dL`,
                      'Glucose Level'
                    ]}
                  />
                  <Legend />
                  <Line
                    type="monotone"
                    dataKey="glucoseLevel"
                    stroke="#A32E76"
                    name="Glucose"
                    dot={{ r: 4 }}
                    activeDot={{ r: 8 }}
                  />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>

          {/* Readings Table */}
          <div className="mt-6 bg-white rounded-xl p-6 shadow-sm">
            <h3 className="text-lg font-semibold mb-4">Historical Readings</h3>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    <th className="text-left py-2">Timestamp</th>
                    <th className="text-left py-2">Glucose Level</th>
                    <th className="text-left py-2">Context</th>
                    <th className="text-left py-2">Category</th>
                  </tr>
                </thead>
                <tbody>
                  {[...readings].reverse().map((reading, index) => {
                    const category = getGlucoseCategory(reading.glucoseLevel, reading.mealContext);
                    return (
                      <tr key={index} className="border-b last:border-0">
                        <td className="py-2">
                          {new Date(reading.timestamp).toLocaleString()}
                        </td>
                        <td className="py-2">{reading.glucoseLevel} mg/dL</td>
                        <td className="py-2">{formatMealContext(reading.mealContext)}</td>
                        <td className={`py-2 ${category.color}`}>
                          {category.label}
                        </td>
                      </tr>
                    )})}
                </tbody>
              </table>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}